...
function buildDecorations(view, blockRanges) {
  ...
  const { codeBlockLines, tableLines, mathBlockLines, headings } = blockRanges;
  const decorations = [];
  ...
  // Collapsed heading lines should always show caret (treated as focused)
  const collapsedHeadingLines = collapsed;
    ...
    for (let i = startLine; i <= endLine; i++) {
      ...
      if (collapseEnabled) {
        ...
        if (headingInfo) {
          ...
          const isCollapsed = collapsedHeadingLines.has(i);

          // If heading is collapsed, always show raw markdown with caret
          if (isCollapsed) {
            continue;
          }
...
export function hybridPreview(options = {}) {
  const { enableCollapse = true } = options;
