 ...
 *   - Toggle (default): `{ label, handler, getState? }` — shows checkmark, stays open on click
 *   - Action: `{ type: 'action', label, handler }` — no checkmark, closes menu on click
 *   - Separator: `{ type: 'separator' }` — renders a horizontal divider line
 * @returns {Extension[]}
 */
    ...
    constructor(view) {
      ...
      for (const item of items) {
        if (item.type === 'separator') {
          ...
          sep.className = 'cm-more-menu-separator';
